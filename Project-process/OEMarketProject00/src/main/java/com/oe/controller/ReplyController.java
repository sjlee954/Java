...
public class ReplyController {
	...
	public ResponseEntity<List<ReplyVO>> getList(@PathVariable("bo_num")Long bo_num, @PathVariable("page") int page) {
		log.info("getList..." + bo_num + " : " + page);
		CriteriaVO cri = new CriteriaVO(page, 10);  // 10개 페이지씩
		log.info("cri : " + cri);
		
		return new ResponseEntity<>(service.getList(cri, bo_num), HttpStatus.OK);
	}
