...
/*
 * 작업       URL      (HTTP전송방식)
 * 등록: /replies/new : Post방식
 * 조회: /replies/:rno : Get방식
 * 삭제: /replies/:rno : DELETE
 * 수정: /replies/:rno : PUT or PATCH
 * 페이지: /replies/pages/:bno/:page : GET방식
 * 
 */
@RestController
...
public class ReplyController {
	...
	//클라이언트에서 서버전송 json(consumes = "application/json") 
	//서버에서 클라이언트 string(produces= {MediaType.TEXT_PLAIN_VALUE} - 반환타입)
	//{속성:값, 속성:값}--> @RequestBody
	//{bno:값, reply:값, replyer:값} 이렇게 넘겨줘야 변환해서 ReplyVO vo가 받을 수 있음!!
	...
	@GetMapping(value="/get/{rno}", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_ATOM_XML_VALUE})
